...
import { Centered, Iconify } from "@ffilip/mui-react-utils";
...
const SingleStatCard = ({ title, subTitle, count, icon, isSmall }: ISingleStatProps) => (
  <Card variant="solid" sx={{ flex: 1, minWidth: 0, p: "12px" }} color="primary" invertedColors>
    <CardContent sx={{ justifyContent: "space-between" }}>
      <Box>
        <Typography level="title-lg" fontSize={isSmall ? 17 : undefined}>{title}</Typography>
        <Typography level="body-sm" fontSize={isSmall ? undefined : 18} lineHeight={isSmall ? "14px" : undefined}>{subTitle}</Typography>
      </Box>
